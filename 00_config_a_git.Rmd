...
À noter : sois on indique un accès https (comme on va le voir ci-ensuite) soit un lien ssh, si on a configuré une clée SSH.
...
2. Et copier-coller l'adresse SSH du projet sur Github pour la reporter dans Rstudio.

#### Initialiser un nouveau projet
Le plus simple pour synchroniser un projet Rstudio et un repo github, c'est généralement d'indiquer l'adresse SSH d'un nouveau repo à la création d'un nouveau projet Rstudio.
...
2. Via l’interface Git de RStudio

Ouvre ton projet dans RStudio

Menu Tools > Project Options > Git/SVN → clique sur Enable version control (si ce n’est pas déjà fait)

Clique ensuite sur l’onglet Git dans le coin supérieur droit de RStudio :

Clique sur Commit, sélectionne tous les fichiers → bouton Commit

Clique ensuite sur la petite roue dentée ⚙️ → Shell...

Dans le shell qui s’ouvre, tape :
